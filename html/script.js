...
function updateFormHeader() {
    const titles = {
        1: 'Cadastre-se Agora',
        2: 'Tipo de Cadastro',
        3: formData.tipoCadastro === 'lojista' ? 'Finalizar Cadastro' : 'Cadastro Exclusivo'
    };
    
    const subtitles = {
        1: 'Comece sua jornada como lojista oficial',
        2: 'Escolha o tipo de cadastro desejado',
        3: formData.tipoCadastro === 'lojista' ? 'Dados da sua empresa' : 'Para lojistas com CNPJ'
    };
...
function updateStep1Button() {
    const isValid = formData.nomeCompleto.trim().length > 0 && formData.whatsapp.trim().length > 0;
...
function showSuccessModal() {
    ...
    if (formData.tipoCadastro === 'lojista') {
        ...
        modalMessage.textContent = 'Nossa equipe entrará em contato via WhatsApp em breve para finalizar sua parceria.';
